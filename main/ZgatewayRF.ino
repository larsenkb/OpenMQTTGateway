...
void txCode(int val, int dly, int rpt)
{
	...
	do {
		...
		do {
			if (digit&val) {
    		digitalWrite(RF_EMITTER_PIN, HIGH);    // set the LED off
		  	delayMicroseconds(dlyh);
     	  digitalWrite(RF_EMITTER_PIN, LOW);    // set the LED off
